...
class AdaptiveHuffman:
    ...
    def encode(self, print_bar):
        ...
        entropy_val = entropy(self.byte_seq)
#        logging.getLogger(__name__).info('entropy: %f', entropy_val)
        ...
        elapsed_time = progressbar.elapsed
        if (print_bar):
            progressbar.finish()
        return bitarray(code), entropy_val, elapsed_time
    ...
    def decode(self):
        ...
        elapsed_time = progressbar.elapsed
        progressbar.finish()
        return (decode_dpcm(code) if self.dpcm else code), elapsed_time
...
def compress(in_filename, out_filename, print_bar, alphabet_range, dpcm):
...
#        )
    ...
    code, entropy, compression_time = ada_huff.encode(print_bar)
...
#    logging.getLogger(__name__).info(
#        'compression ratio: %.5f', os.path.getsize(in_file.name) / os.path.getsize(out_filename)
#    )
    original_size = os.path.getsize(in_file.name)
    compression_ratio = os.path.getsize(in_file.name) / os.path.getsize(out_filename)
    return original_size, compression_time, compression_ratio, entropy
...
#        )
    ada_huff = AdaptiveHuffman(content, alphabet_range, dpcm)
    code, decompression_time = ada_huff.decode()
...
#    )
    compressed_size = os.path.getsize(in_file.name)
    compression_ratio = os.path.getsize(out_filename) / os.path.getsize(in_file.name)

    return compressed_size, compression_ratio, decompression_time
