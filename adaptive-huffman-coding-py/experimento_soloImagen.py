...
def main():
    ...
    measures_file = "../csv-intermedio/measures_concated.csv"
    ...
    for x in range(1, num_samples+1):
        for idx in range(1, x+1):
            ...
            if (idx == 1):
                big_data = data_stream
            else:
                big_data = big_data + data_stream


        with open("results/data_combined.raw", "wb") as big_file:
            big_file.write(big_data)     

        original_size, compression_time, compression_ratio, entropy = compress("results/data_combined.raw", "results/data_combined_compress",
                True, alphabet_range=alphabet_range, dpcm=dpcm)
        # compressed_size, compression_ratio, decompression_time = extract("results/data_combined_compress", "results/data_combined_decompress.raw",
        #         alphabet_range=alphabet_range, dpcm=dpcm)
